...
int main (int argc, char **argv) {
  ...
  if(id == 0) {
	...
	MPI_File_open(MPI_COMM_SELF,F,MPI_MODE_WRONLY |MPI_MODE_APPEND,MPI_INFO_NULL,&ft);
    sprintf(text,"Mpi,%lf\n",(double)milisegundos / 1000.0);
	MPI_File_write(ft,text,strlen(text), MPI_CHAR,&status);
	MPI_File_close(&ft);
    // Liberamos memoria
    free(datos);
  }
