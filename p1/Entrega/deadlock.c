 ...
 *
 * Version modificada de deadlock.c para que no entre en interbloqueo
 * (Ejercicio 2.2.3 y 2.2.4)
     ...
     char **argv;
{
  ...
  if(rank == 1){
      MPI_Recv ( message1, MSGLEN, MPI_FLOAT, source, recv_tag, MPI_COMM_WORLD,
	    &status );
 }else{
	MPI_Send ( message1, MSGLEN, MPI_FLOAT, dest, send_tag, MPI_COMM_WORLD );
